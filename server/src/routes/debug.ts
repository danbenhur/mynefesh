...
// Public — one-time wipe of all umbrella data for clean-slate testing.
...
router.post('/wipe-all-umbrellas-public', async (_req, res) => {
  try {
    ...
    const deletedReminders = await db.delete(reminders).returning({ id: reminders.id })
    ...
    // Self-referential: delete children before roots to avoid cascade ambiguity.
