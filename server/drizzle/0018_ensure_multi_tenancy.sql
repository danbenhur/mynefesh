-- Catch-all: re-applies all of 0017's DDL idempotently.
-- Purpose: if 0017 was marked as applied by the legacy migration-seeder without its DDL
-- actually running, this migration ensures the multi-tenancy schema is present regardless.
-- Safe to run on any DB state: fully migrated (all no-ops), partially migrated, or fresh.
-- Difference from 0017: TRUNCATE api_usage is omitted (spend logs preserved).
...
-- =====================================================================
-- 5. api_usage: RETYPE user_id column (no TRUNCATE — preserve spend logs)
-- Only drops and re-adds if column is text (legacy google-profile-ID type).
-- If already uuid, DROP+ADD is still safe (logging table, no business value).
-- =====================================================================
